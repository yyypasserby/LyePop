...
bool mysqlUserValidate( MYSQL * mysql, User * user ){
    MYSQL_RES * res;
    char query[ bufsize ];
    sprintf( query, "SELECT * FROM user WHERE username='%s' AND password='%s'", user->username, user->password );
...
void mysqlUserLogin( MYSQL * mysql, User * user ){
    char query[ bufsize ];
    sprintf( query, "INSERT userol SET username='%s', ipaddress='%s'", user->username, user->ipaddress );
    mysqlQuery( mysql, query );
}
