...
MYSQL *mysqlInit() {
  ...
  if (mysql_real_connect(mysql, "localhost", "root", "root", "linpop", 0, NULL,
                         0) == NULL) {
...
bool mysqlUserValidate(MYSQL *mysql, User *user) {
  ...
  char query[bufsize];
  sprintf(query,
          "SELECT nickname,signname FROM user WHERE username='%s' AND "
          "password='%s'",
...
void mysqlUserLogin(MYSQL *mysql, User *user) {
  char query[bufsize];
  sprintf(query, "REPLACE userol SET username='%s', ipaddress='%s'",
...
void mysqlUserSetInfo(MYSQL *mysql, User *user) {
  char query[bufsize];
  sprintf(query,
          "UPDATE user SET nickname='%s', signname='%s' WHERE username='%s'",
          user->nickname, user->signname, user->username);
  mysqlQuery(mysql, query);
}
...
void mysqlUserGetInfo(MYSQL *mysql, User *user) {
  ...
  MYSQL_ROW user_attr;
  char query[bufsize];
  sprintf(query, "SELECT nickname,signname FROM user WHERE username='%s'",
          user->username);
  res = mysqlQuery(mysql, query);
  if ((user_attr = mysql_fetch_row(res)) == NULL) {
    user = NULL;
...
void mysqlUserGetInfoById(MYSQL *mysql, char *id, User *user) {
  ...
  MYSQL_ROW user_attr;
  char query[bufsize];
  sprintf(query, "SELECT username, nickname, signname FROM user WHERE id='%s'",
          id);
  res = mysqlQuery(mysql, query);
  if ((user_attr = mysql_fetch_row(res)) == NULL) {
    user = NULL;
...
void mysqlUserGetFriendList(MYSQL *mysql, char *username, User **friend_list,
  ...
  sprintf(query, "SELECT to_user_id FROM user_friends WHERE from_user_id='%s'",
          userid);
  res = mysqlQuery(mysql, query);
...
void mysqlUserAddFriend(MYSQL *mysql, char *username, char *friendname) {
  ...
  sprintf(query,
          "REPLACE INTO user_friends( from_user_id, to_user_id ) values ('%s', "
          "'%s')",
          userid, friendid);
  mysqlQuery(mysql, query);
}
...
void mysqlUserChangePWD(MYSQL *mysql, User *user) {
  char query[bufsize];
  sprintf(query, "UPDATE user SET password='%s' WHERE username='%s'",
