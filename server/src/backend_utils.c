...
void loginResponse(cJSON* function, char* sendBuffer) {
  ...
  strcpy(user.ipaddress,
         cJSON_GetObjectItem(function, "ipaddress")->valuestring);
  strcpy(user.username,
         cJSON_GetObjectItem(jsonuser_info, "username")->valuestring);
  strcpy(user.password,
         cJSON_GetObjectItem(jsonuser_info, "password")->valuestring);
...
void getfriendResponse(cJSON* function, char* sendBuffer) {
  ...
  for (i = 0; i < friend_list_len; ++i) {
    if (mysqlUserGetIp(mysql, friend_list[i].username, t_ipaddress)) {
      ...
      cJSON_AddItemToArray(jsonfriend,
                           cJSON_CreateString(friend_list[i].username));
      cJSON_AddItemToArray(jsonfriend,
                           cJSON_CreateString(friend_list[i].nickname));
      cJSON_AddItemToArray(jsonfriend,
                           cJSON_CreateString(friend_list[i].signname));
...
void searchResponse(cJSON* function, char* sendBuffer) {
  ...
  cJSON* jsonresponse = cJSON_CreateObject();
  strcpy(user->username,
         cJSON_GetObjectItem(function, "username")->valuestring);
  mysqlUserGetInfo(mysql, user);
  if (NULL == user)
    cJSON_AddNumberToObject(jsonresponse, "result", ON_FAILURE);
  else {
    cJSON* jsonuser_info;
    cJSON_AddItemToObject(jsonresponse, "user_info",
                          jsonuser_info = cJSON_CreateObject());
    cJSON_AddNumberToObject(jsonresponse, "result", ON_SUCCESS);
