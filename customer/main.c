...
static GtkWidget *dataWindow1;              //main
...
static GtkWidget* chatWindow;                  ///chat
...
static GtkWidget* friendManageTreeView;                      //friend manage
...
static gint selfIcon=16;                             //ziji touxiang
...
info closeFriend[3]={{1,"aaa","aaaaa","aaaaaa",1},{2,"bbbb","bbbbbb","bbbbb",1},{3,"ccc","ccccccc","cccc",1}};
info friendlist1[3]={{1,"aaa","aaaaa","aaaaaa",1},{2,"bbbb","bbbbbb","bbbbb",1},{3,"ccc","ccccccc","cccc",1}};
info friendlist2[3]={{1,"aaa","aaaaa","aaaaaa",0},{2,"bbbb","bbbbbb","bbbbb",0},{3,"ccc","ccccccc","cccc",0}};
...
//            path= g_strdup_printf("./picture/mainbutton/mainbutton%d.png",(int)data);
...
void create_background_window(){
    ...
    backPixbuf = gdk_pixbuf_new_from_file("./picture/mainbutton/workspace2.png", NULL);       // gdk函数读取png文件 
    gdk_pixbuf_render_pixmap_and_mask(backPixbuf, &backPixmap, &backBitmap, 128);    // alpha小于128认为透明 
    gtk_widget_shape_combine_mask(backwindow, backBitmap, 0, 0);          // 设置透明蒙板 
    gdk_window_set_back_pixmap(backwindow->window, backPixmap, FALSE);          // 设置窗口背景 
    ...
    g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(destroy_all_window),NULL);
    //g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(gtk_widget_destroy_all),backwindow);
...
void create_dataWindow1(){
    ...
    gtk_window_move(GTK_WINDOW(dataWindow1),mainWinX-datawinxadd,mainWinY-datawinyadd);
    gtk_widget_set_app_paintable(dataWindow1, TRUE); 
    gtk_widget_realize(dataWindow1); 

    data1Pixbuf = gdk_pixbuf_new_from_file("./picture/mainbutton/leftframe.png", NULL);       // gdk函数读取png文件 
    gdk_pixbuf_render_pixmap_and_mask(data1Pixbuf, &data1Pixmap, &data1Bitmap, 128);    // alpha小于128认为透明 
    gtk_widget_shape_combine_mask(dataWindow1, data1Bitmap, 0, 0);          // 设置透明蒙板 
    gdk_window_set_back_pixmap(dataWindow1->window, data1Pixmap, FALSE);          // 设置窗口背景 
    ...
    gchar* iconPath = g_strdup_printf("./picture/photograph/%db.jpg",closeFriend[0].icon);
...
void create_dataWindow2(){
    ...
    gtk_window_move(GTK_WINDOW(dataWindow2),mainWinX-datawinxadd,mainWinY-datawinyadd+80);
    gtk_widget_set_app_paintable(dataWindow2, TRUE); 
    //gtk_widget_realize(dataWindow); 

    data2Pixbuf = gdk_pixbuf_new_from_file("./picture/mainbutton/leftframe.png", NULL);       // gdk函数读取png文件 
    gdk_pixbuf_render_pixmap_and_mask(data2Pixbuf, &data2Pixmap, &data2Bitmap, 128);    // alpha小于128认为透明 
    gtk_widget_shape_combine_mask(dataWindow2, data2Bitmap, 0, 0);          // 设置透明蒙板 
    gdk_window_set_back_pixmap(dataWindow2->window, data2Pixmap, FALSE);          // 设置窗口背景 
    ...
    gchar* iconPath = g_strdup_printf("./picture/photograph/%db.jpg",closeFriend[1].icon);
    image = gtk_image_new_from_file(iconPath);
    gtk_container_add(GTK_CONTAINER(box1),image);
    gtk_box_pack_start(GTK_BOX(box2),label2,FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(box2),label22,FALSE,FALSE,0);
...
void create_dataWindow3(){
    ...
    gtk_window_move(GTK_WINDOW(dataWindow3),mainWinX-datawinxadd,mainWinY-datawinyadd+160);
    ...
    data3Pixbuf = gdk_pixbuf_new_from_file("./picture/mainbutton/leftframe.png", NULL);       // gdk函数读取png文件 
    gdk_pixbuf_render_pixmap_and_mask(data3Pixbuf, &data3Pixmap, &data3Bitmap, 128);    // alpha小于128认为透明 
    gtk_widget_shape_combine_mask(dataWindow3, data3Bitmap, 0, 0);          // 设置透明蒙板 
    gdk_window_set_back_pixmap(dataWindow3->window, data3Pixmap, FALSE);          // 设置窗口背景 
    ...
    gchar* iconPath = g_strdup_printf("./picture/photograph/%db.jpg",closeFriend[2].icon);
    image = gtk_image_new_from_file(iconPath);
    gtk_container_add(GTK_CONTAINER(box1),image);
    gtk_box_pack_start(GTK_BOX(box2),label2,FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(box2),label22,FALSE,FALSE,0);
...
void create_main_window(){
    ...
    mainPixbuf = gdk_pixbuf_new_from_file("mainbackground2.png", NULL);       // gdk函数读取png文件 
    gdk_pixbuf_render_pixmap_and_mask(mainPixbuf, &mainPixmap, &mainBitmap, 128);    // alpha小于128认为透明 
    gtk_widget_shape_combine_mask(mainWindow, mainBitmap, 0, 0);          // 设置透明蒙板 
    gdk_window_set_back_pixmap(mainWindow->window, mainPixmap, FALSE);          // 设置窗口背景 l
...
//    g_signal_connect(G_OBJECT(recent_button1),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)1);
//    g_signal_connect(G_OBJECT(recent_button1),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)1);
    g_signal_connect(G_OBJECT(recent_button1),"enter_notify_event",G_CALLBACK(button_event1),NULL);
    g_signal_connect(G_OBJECT(recent_button1),"leave_notify_event",G_CALLBACK(button_event1),NULL);
//    g_signal_connect(G_OBJECT(recent_button1),"clicked",G_CALLBACK(quick_begin_to_chat),NULL);
    g_signal_connect(G_OBJECT(recent_button1),"clicked",G_CALLBACK(destroy_all_window),NULL);
//
//    g_signal_connect(G_OBJECT(recent_button2),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)2);
//    g_signal_connect(G_OBJECT(recent_button2),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)2);
    g_signal_connect(G_OBJECT(recent_button2),"enter_notify_event",G_CALLBACK(button_event2),NULL);
    g_signal_connect(G_OBJECT(recent_button2),"leave_notify_event",G_CALLBACK(button_event2),NULL);
//    g_signal_connect(G_OBJECT(recent_button2),"clicked",G_CALLBACK(quick_begin_to_chat),NULL);
    g_signal_connect(G_OBJECT(recent_button2),"clicked",G_CALLBACK(destroy_all_window),NULL);
//
//    g_signal_connect(G_OBJECT(recent_button3),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)3);
//    g_signal_connect(G_OBJECT(recent_button3),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)3);
    g_signal_connect(G_OBJECT(recent_button3),"enter_notify_event",G_CALLBACK(button_event3),NULL);
    g_signal_connect(G_OBJECT(recent_button3),"leave_notify_event",G_CALLBACK(button_event3),NULL);
//    g_signal_connect(G_OBJECT(recent_button3),"clicked",G_CALLBACK(quick_begin_to_chat),NULL);
    g_signal_connect(G_OBJECT(recent_button3),"clicked",G_CALLBACK(destroy_all_window),NULL);
//
//
//
//    g_signal_connect(G_OBJECT(more_button),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)4);
//    g_signal_connect(G_OBJECT(more_button),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)4);
    g_signal_connect(G_OBJECT(more_button),"clicked",G_CALLBACK(button_event4),NULL);
//
//    g_signal_connect(G_OBJECT(close_button),"button_press_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)5);
//    g_signal_connect(G_OBJECT(close_button),"button_release_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)5);
//    g_signal_connect(G_OBJECT(close_button),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)5);
//    g_signal_connect(G_OBJECT(close_button),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)5);
    g_signal_connect(G_OBJECT(close_button),"clicked",G_CALLBACK(button_event5),NULL);
...
void create_set_menu(){
    ...
    //g_signal_connect(G_OBJECT(menuWindow),"destroy",G_CALLBACK(gtk_main_quit),NULL);
    gtk_widget_set_app_paintable(menuWindow, TRUE); 
    gtk_widget_realize(menuWindow); 

    pixbuf = gdk_pixbuf_new_from_file("./picture/menubutton/menubackground.png", NULL);       // gdk函数读取png文件 

    gdk_pixbuf_render_pixmap_and_mask(pixbuf, &pixmap, &bitmap, 128);    // alpha小于128认为透明 

    gtk_widget_shape_combine_mask(menuWindow, bitmap,0,0);          // 设置透明蒙板 

    gdk_window_set_back_pixmap(menuWindow->window, pixmap, FALSE);          // 设置窗口背景 
...
//    g_signal_connect(G_OBJECT(friendListButton),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)6);
//    g_signal_connect(G_OBJECT(friendListButton),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)6);
//
//    g_signal_connect(G_OBJECT(friendManageButton),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)7);
//    g_signal_connect(G_OBJECT(friendManageButton),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)7);
//
//    g_signal_connect(G_OBJECT(friendSearchButton),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)8);
//    g_signal_connect(G_OBJECT(friendSearchButton),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)8);
//
//    g_signal_connect(G_OBJECT(settingButton),"enter_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)9);
//    g_signal_connect(G_OBJECT(settingButton),"leave_notify_event",GTK_SIGNAL_FUNC(mouse_event_handle),(gpointer)9);

    //g_signal_connect(G_OBJECT(friendListButton),"clicked",G_CALLBACK(createFriendWindow),NULL);
    g_signal_connect_swapped(G_OBJECT(friendListButton),"clicked",G_CALLBACK(destroy_all_window),NULL);

    //g_signal_connect(G_OBJECT(friendManageButton),"clicked",G_CALLBACK(create_friend_manage_window),NULL);
    g_signal_connect_swapped(G_OBJECT(friendManageButton),"clicked",G_CALLBACK(destroy_all_window),NULL);

    //g_signal_connect(G_OBJECT(friendSearchButton),"clicked",G_CALLBACK(create_search_window),NULL);
    g_signal_connect_swapped(G_OBJECT(friendSearchButton),"clicked",G_CALLBACK(destroy_all_window),NULL);
