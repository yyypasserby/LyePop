...
void * serverRecvThread( void * arg ){
    ...
    int type, ret ;
    while(1){
        if( (ret = recvPackage( &connectfd, recv_buffer, bufsize )) ==0 )
            continue;
        printf( "%s\n", recv_buffer );
...
void * listenThread( void * arg ){
    ...
    while(1){
        ...
        if( FUNC_MESSAGE == cJSON_GetObjectItem( json_message, "header" )->valueint ){
            ...
            sprintf( _title, "%s!", cJSON_GetObjectItem( json_friend, "username" )->valuestring );
            sprintf( _content, "\nYour friend %s is online now!", cJSON_GetObjectItem( json_friend, "username" )->valuestring );
        ...
        strcpy( from_user, cJSON_GetObjectItem( json_message, "username" )->valuestring );
        ...
        for( i = 0; i < current_pid_len; ++i ){
            ...
            if( strcmp( ( cJSON_GetObjectItem( json_user, "destname" ) -> valuestring ), from_user ) == 0 ){
                ...
                char * sendtime = cJSON_GetObjectItem( json_message, "sendtime" )->valuestring;
                char * content = cJSON_GetObjectItem( json_message, "content" )->valuestring;
                printf( "message : %s\n", content );
        ...
        if( i == current_pid_len ){ 
            ...
            createDetachPthread( &createpid_thread, createPidThreadNotShow, ( void * )json_user_info );
        }
    }
    pthread_exit( NULL );
}
...
void * getfriendThread( void * arg ){
    ...
    if( result == ON_SUCCESS ){
        ...
        for( i = 0; i < friend_ol_list_len; ++i ){
            ...
            initAddr( &dest_addr, cJSON_GetArrayItem( json_friend, 3 )->valuestring, listenport );
            printf( "%s\n", current_user_info );
            ...
            strcpy( send_buffer, cJSON_Print( function ) );
            printf( "%s\n", send_buffer );
            sendMessageTo( &sendFuncfd, send_buffer, &dest_addr, strlen( send_buffer ) + 1 );
        }
    }
    else{
