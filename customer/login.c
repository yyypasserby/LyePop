/*************************************************************************
    ...
    > Created Time: 2013年09月12日 星期四 13时40分10秒
 ************************************************************************/
...
void * sendLoginRequest( void * arg ){
    ...
    cJSON_AddStringToObject( user, "username", gtk_entry_get_text( GTK_ENTRY( name_input ) ) );
    cJSON_AddStringToObject( user, "password", gtk_entry_get_text( GTK_ENTRY( pwd_input ) ) );
...
void createLoginWindow(){
    ...
    g_signal_connect(G_OBJECT(login_window), "button-press-event",G_CALLBACK(onMove), NULL);

    pixbuf = gdk_pixbuf_new_from_file("./layout/loginBG.png", NULL);       // gdk函数读取png文件 
    ...
    gdk_window_set_back_pixmap( login_window->window, pixmap, FALSE );          // 设置窗口背景 
    ...
    register_button_image = gtk_image_new_from_file("./layout/registerbutton.png");
    ...
    g_signal_connect(G_OBJECT(login_button),"clicked",G_CALLBACK(onLogin),NULL);
    g_signal_connect(G_OBJECT(register_button),"clicked",G_CALLBACK(onRegister),NULL);

    g_signal_connect_swapped(G_OBJECT(close_button),"clicked",G_CALLBACK( onQuit ),login_window);
