...
//void remove_from_friend_list(GtkWidget *widget, gpointer view)//删除好友  只删除列表中数据
//{
//    gchar *value;
//    GtkTreeModel *model;
//    GtkTreeIter iter;
//    model = gtk_tree_view_get_model (GTK_TREE_VIEW (view));
//    if(gtk_tree_selection_get_selected(GTK_TREE_SELECTION(view),&model,&iter)){
...
void add_to_friend_list( cJSON * json_friend ){
    GtkTreeModel *model;
    GtkTreeIter iter;
    ...
    strcpy( _ipaddress, cJSON_GetObjectItem( json_friend, "ipaddress" )->valuestring);
    ...
    gtk_list_store_set( GTK_LIST_STORE( model ), &iter, ICON_COLUMN, _icon, NAME_COLUMN, _username, EXTRA_COLUMN, _ipaddress, -1);
    gtk_tree_view_set_model( GTK_TREE_VIEW( listOnlineFriendTreeView ), model );
}

void list_begin_chat(GtkTreeView *treeview,GtkTreePath *path,GtkTreeSelection *selection){
    ...
    for( i = 0; i < current_pid_len; ++i ){
        ...
        if( strcmp( ( cJSON_GetObjectItem( json_user, "destname" ) -> valuestring ), from_user ) == 0 ){
    ...
    if( i == current_pid_len ){ 
        ...
        cJSON_AddStringToObject( json_user_info, "username", cJSON_GetObjectItem( cJSON_Parse( current_user_info ), "username" )->valuestring );
        ...
        createDetachPthread( &createpid_thread, createPidThreadShow, ( void * )json_user_info );
    }
    //pthread_t createpid_thread;
    //createDetachPthread( &createpid_thread, createPidThreadShow, NULL );
    g_print("%s\n", from_user );
}
...
void createPidThreadShow( void * arg ){
    ...
    strcpy( username, cJSON_GetObjectItem( json_user_info ,"username" )->valuestring );
    strcpy( destname, cJSON_GetObjectItem( json_user_info ,"destname" )->valuestring );
    strcpy( ipaddress, cJSON_GetObjectItem( json_user_info ,"ipaddress" )->valuestring );
...
GtkTreeModel* create_friend_list_model( cJSON * _friend_list, int type ){
    ...
    cJSON * _friend;
    treestore=gtk_list_store_new( NUM_COLUMN, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING );
    for( i = 0; i < size; i++ ){
        GdkPixbuf* icon;
        ...
        if ( type == 0 ){
            ...
            printf("%s\n", ipaddress);
            gtk_list_store_set (treestore, &iter, ICON_COLUMN, icon, NAME_COLUMN, username, EXTRA_COLUMN, ipaddress, -1);
        }
        else{
            iconName = g_strdup_printf("./head_icon/%sh.jpg", username );
            icon = gdk_pixbuf_new_from_file(iconName, NULL);
            strcpy( nickname, cJSON_GetArrayItem( _friend, 1 )->valuestring );
            gtk_list_store_set (treestore, &iter, ICON_COLUMN, icon, NAME_COLUMN, username, EXTRA_COLUMN, nickname, -1);
        }
        g_free(iconName);
    }            
...
void set_friend_list_columns(GtkTreeView* view){
    ...
    gtk_tree_view_column_add_attribute(column,cellRenderer,"pixbuf",ICON_COLUMN);
    ...
    cellRenderer=gtk_cell_renderer_text_new();
    gtk_tree_view_column_pack_start(column,cellRenderer,FALSE);
    gtk_tree_view_column_add_attribute(column,cellRenderer,"text",EXTRA_COLUMN);
    gtk_tree_view_append_column(GTK_TREE_VIEW(view),column);
}

void updateFriendList( cJSON * _friend_list, GtkWidget * _friend_list_tree_view, int type ){
...
////void onFriendList(){
//    gint num=G_N_ELEMENTS(friendlist1);
//    GtkTreeModel* model=create_friend_list_model(friendlist1,num);
//    gtk_tree_view_set_model(GTK_TREE_VIEW(listOnlineFriendTreeView),model);
//    set_friend_list_columns(GTK_TREE_VIEW(listOnlineFriendTreeView));
//}
//
//
//void downFriendList(){
//    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(listOfflineFriendTreeView),FALSE);
//    gint num=G_N_ELEMENTS(friendlist2);
//    GtkTreeModel* model=create_friend_list_model(friendlist2,num);
//    gtk_tree_view_set_model(GTK_TREE_VIEW(listOfflineFriendTreeView),model);
//    set_friend_list_columns(GTK_TREE_VIEW(listOfflineFriendTreeView));
//}
...
void createFriendWindow(){
    ...
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(listOnlineFriendTreeView),FALSE);
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(listOfflineFriendTreeView),FALSE);
    set_friend_list_columns(GTK_TREE_VIEW(listOnlineFriendTreeView));
    set_friend_list_columns(GTK_TREE_VIEW(listOfflineFriendTreeView));

    GtkTreeSelection *selection;
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(listOnlineFriendTreeView));
    g_signal_connect(listOnlineFriendTreeView, "row-activated", G_CALLBACK(list_begin_chat), selection);
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(listOfflineFriendTreeView));
    g_signal_connect(listOfflineFriendTreeView, "row-activated", G_CALLBACK(list_begin_chat), NULL);
    ...
    g_signal_connect(G_OBJECT( friendListWindow),"destroy",G_CALLBACK( onDestroy ), friendListWindow );
