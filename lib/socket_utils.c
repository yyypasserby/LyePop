...
void zeroinAddr(struct sockaddr_in *addr) { memset(addr, 0, sizeof(*addr)); }
...
void getAccept(int *socketfd, int *connect_socketfd,
  ...
  if ((*connect_socketfd = accept(*socketfd, (struct sockaddr *)cli_addr,
...
void getConnect(int *send_socketfd, struct sockaddr_in *pin_addr) {
  if (connect(*send_socketfd, (struct sockaddr *)pin_addr, sizeof(*pin_addr)) ==
      -1) {
...
int recvPackageFrom(int *connectfd, void *buf, struct sockaddr_in *send_addr,
  ...
  if ((ret = recvfrom(*connectfd, buf, bufsize, 0, (struct sockaddr *)send_addr,
                      &addr_len)) == -1) {
    perror("can't recv from other client!\n");
  }
  printf("recieve from %s:%d : %s\n", inet_ntoa(send_addr->sin_addr),
         ntohs(send_addr->sin_port), (char *)buf);
  sleep(1);
}
...
void sendMessageTo(int *connectfd, void *buf, struct sockaddr_in *dest_addr,
                   int bufsize) {
  int addr_len = sizeof(struct sockaddr_in);
  if (sendto(*connectfd, buf, bufsize, 0, (struct sockaddr *)dest_addr,
             addr_len) == -1) {
