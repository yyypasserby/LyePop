...
int progress_timeout(gpointer data) {
  ...
  if (pdata->activity_mode)
    ...
    new_val =
        gtk_progress_bar_get_fraction(GTK_PROGRESS_BAR(pdata->pbar)) + 0.01;
...
void open_file(GtkWidget *button, gpointer userdata) {
  ...
  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(dialog)->ok_button),
                     "clicked", GTK_SIGNAL_FUNC(on_file_select_ok),
...
void select_face(GtkButton *button1, gpointer data) {
  ...
  gtk_table_attach(GTK_TABLE(table), button, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)1);

  button = gtk_button_new();
  image = gtk_image_new_from_file("./face/face2.gif");
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 1, 2, 0, 1, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)2);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 2, 3, 0, 1, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)3);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 3, 4, 0, 1, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)4);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)5);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 1, 2, 1, 2, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)6);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 2, 3, 1, 2, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)7);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 3, 4, 1, 2, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)8);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)9);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 1, 2, 2, 3, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)10);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 2, 3, 2, 3, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)11);
  ...
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_table_attach(GTK_TABLE(table), button, 3, 4, 2, 3, GTK_FILL, GTK_FILL, 0,
                   0);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_face_button),
                   (gpointer)12);
...
void create_record_window() {
  ...
  g_signal_connect(G_OBJECT(record_window), "button-press-event",
                   G_CALLBACK(onMove), NULL);
  pixbuf = gdk_pixbuf_new_from_file("chatwindow.png", NULL);
  gdk_pixbuf_render_pixmap_and_mask(pixbuf, &pixmap, &bitmap, 128);
  gtk_widget_shape_combine_mask(record_window, bitmap, 0, 0);
  gdk_window_set_back_pixmap(record_window->window, pixmap, FALSE);
  ...
  gtk_text_view_set_left_margin(GTK_TEXT_VIEW(record_textview), 10);
  gtk_text_view_set_right_margin(GTK_TEXT_VIEW(record_textview), 10);
...
void control_rec_scrolled_win() {
  ...
  adj = gtk_scrolled_window_get_vadjustment(
      GTK_SCROLLED_WINDOW(Rcv_scrolled_win));
  ...
  if (to_value < lower_value) to_value = lower_value;
...
void Get_Local_message(const gchar *text) {
  GtkTextIter start, end;
  gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(Send_buffer), &start, &end);
  gtk_text_buffer_insert(GTK_TEXT_BUFFER(Send_buffer), &end, text,
                         strlen(text));
}
...
void Put_Remote_message(cJSON *_json_message) {
  ...
  gtk_text_buffer_insert(GTK_TEXT_BUFFER(Rcv_buffer), &end, usertime,
                         strlen(usertime));
  gtk_text_buffer_insert(GTK_TEXT_BUFFER(Rcv_buffer), &end, _content,
                         strlen(_content));
  gtk_text_buffer_insert(GTK_TEXT_BUFFER(Rcv_buffer), &end, "\n", 1);
  control_rec_scrolled_win();
  control_rec_scrolled_win();
...
void on_send(GtkButton *SaveButton, GtkWidget *Send_textview) {
  ...
  gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(Send_buffer), &start, &end);
  R_text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(Send_buffer), &start, &end,
                                    FALSE);
  ...
  if (strcmp(R_text, "") != 0) {
    ...
    Show_Err("Message can't be empty.\n");
  free(R_text);
  control_rec_scrolled_win();
  control_rec_scrolled_win();
}
...
void createChatWindow() {
  ...
  GtkWidget *box, *chat_box, *close_box;
  GtkWidget *left_box, *right_box, *middle_box;
  GtkWidget *Button_Box1, *Button_Box2, *SaveButton, *CloseButton, *FileButton,
      *FontButton, *ColorButton, *FaceButton, *RecordButton, *close;
  ...
  gtk_window_set_title(GTK_WINDOW(chat_window), "Save Text");
  gtk_container_set_border_width(GTK_CONTAINER(chat_window), 10);
  gtk_widget_set_size_request(chat_window, 400, 450);
  ...
  g_signal_connect(G_OBJECT(chat_window), "button-press-event",
                   G_CALLBACK(onMove), NULL);

  pixbuf = gdk_pixbuf_new_from_file("./layout/chatwindow.png", NULL);
  gdk_pixbuf_render_pixmap_and_mask(pixbuf, &pixmap, &bitmap, 128);
  gtk_widget_shape_combine_mask(chat_window, bitmap, 0, 0);
  gdk_window_set_back_pixmap(chat_window->window, pixmap, FALSE);
  ...
  gtk_text_view_set_justification(GTK_TEXT_VIEW(Send_textview),
                                  GTK_JUSTIFY_LEFT);
  ...
  Button_Box1 = gtk_hbutton_box_new();
  gtk_box_set_spacing(GTK_BOX(Button_Box1), 1);
  gtk_button_box_set_layout(GTK_BUTTON_BOX(Button_Box1), GTK_BUTTONBOX_END);
  gtk_container_set_border_width(GTK_CONTAINER(Button_Box1), 5);
  ...
  gtk_widget_add_accelerator(SaveButton, "clicked", gag, GDK_E,
                             GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
  gtk_window_add_accel_group(GTK_WINDOW(chat_window), gag);
  g_signal_connect(G_OBJECT(SaveButton), "clicked", G_CALLBACK(on_send),
                   (gpointer)Send_textview);

  RecordButton = gtk_button_new_with_label("Record");
  g_signal_connect(G_OBJECT(RecordButton), "clicked",
                   G_CALLBACK(on_record_button), NULL);
  ...
  g_signal_connect(G_OBJECT(FileButton), "clicked", G_CALLBACK(open_file),
                   NULL);
  ...
  g_signal_connect(G_OBJECT(FontButton), "clicked", G_CALLBACK(select_font),
                   NULL);
  ...
  g_signal_connect(G_OBJECT(ColorButton), "clicked", G_CALLBACK(select_color),
                   NULL);
  ...
  g_signal_connect(G_OBJECT(FaceButton), "clicked", G_CALLBACK(select_face),
                   NULL);

  close = gtk_button_new_with_label("x");
  g_signal_connect(G_OBJECT(close), "clicked", G_CALLBACK(on_close),
                   (gpointer)Send_textview);
  ...
  gtk_box_pack_start(GTK_BOX(chat_box), Button_Box2, FALSE, FALSE, 2);
  gtk_box_pack_start(GTK_BOX(chat_box), Send_scrolled_win, FALSE, FALSE, 2);
  gtk_box_pack_end(GTK_BOX(close_box), close, FALSE, FALSE, 0);
  gtk_box_pack_end(GTK_BOX(middle_box), Button_Box1, TRUE, TRUE, 5);
  ...
  gtk_container_add(GTK_CONTAINER(chat_window), box);
